...
public class ProjectSetupPage {
 ...
 // Locators for Cost Sheet
    ...
    // ⭐ NEW LOCATORS for Construction Pricing (assuming logical IDs)
    public final By BASE_CONSTRUCTION_PRICE_FIELD = By.id("constructionPrice"); 
    public final By CONSTRUCTION_TAX_RATE_FIELD = By.id("constructionGST");   
 ...
 // NEW LOCATOR: Next Payment Schedule button
    ...
    public void fillEntireForm(ProjectDataModal data) throws InterruptedException, RuntimeException, TimeoutException {
        ...
        // This method is now updated to handle the two new construction pricing fields
...
//        }
 ...
 // ======================= LOCATION DETAILS =======================
    ...
    /**
     * Fills the Cost Sheet details, including both Sales Pricing and the new 
     * Construction Pricing fields.
     */
    public void fillCostSheet(ProjectDataModal.CostSheetDetails costSheet) {
        try {
            ...
            // ⭐ ADDED WAIT: Wait for Base Price to stabilize
            Thread.sleep(5000); 
            
            // 2. Standard Tax (Sales)
            wait.until(ExpectedConditions.visibilityOfElementLocated(STANDARD_TAX_FIELD))
                .sendKeys(costSheet.getStandardTaxRate());
                
            // === ⭐ NEW FIELDS: Construction Pricing ⭐ ===
            // 3. Base Construction Price
            wait.until(ExpectedConditions.visibilityOfElementLocated(BASE_CONSTRUCTION_PRICE_FIELD))
                .sendKeys(costSheet.getBaseConstructionPricePerSqft());
            
            // ⭐ ADDED WAIT: Wait for Construction Price to stabilize
            Thread.sleep(5000); 
            
            // 4. Construction Tax Rate
            wait.until(ExpectedConditions.visibilityOfElementLocated(CONSTRUCTION_TAX_RATE_FIELD))
                .sendKeys(costSheet.getConstructionTaxRate());
            // ===========================================

            System.out.println("✅ Cost Sheet values entered: Base Price (Sales) = " 
                + costSheet.getBasePricePerSqft() + ", GST (Sales) = " + costSheet.getStandardTaxRate() +
                ", Base Price (Const) = " + costSheet.getBaseConstructionPricePerSqft() + 
                ", GST (Const) = " + costSheet.getConstructionTaxRate());
            ...
            // Optional: wait until some confirmation appears (Existing wait)
            Thread.sleep(5000);
 ...
 // ======================= ADD CHARGES =======================
    ...
    public void deleteChargeByName(String chargeName, String modalAction) { 
            
            // ⭐ FIX: Target the first column (td[1]) which corresponds to "Charges(sqft)"
            // This prevents matching text from the 'Category' column.
            String itemRowXPath = 
                "(//td[1][normalize-space(text())='%s']/ancestor::tr)[last()]";
            By itemRowLocator = By.xpath(String.format(itemRowXPath, chargeName));
            ...
            // Locate the item's row element (used for scrolling)
            WebElement itemRowElement = driver.findElement(itemRowLocator);
            
            // ⭐ CRITICAL FIX: Scroll the element into view before interacting with its action button ⭐
            System.out.println("ACTION: Scrolling to make the charge item visible: " + chargeName);
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block: 'center'});", itemRowElement);
            
            // Locator for the action button (three dots)
            String actionButtonXPath = String.format(
                itemRowXPath + "//td[last()]//button", chargeName
            );
            ...
            // ⭐ Specific locator for the confirmation modal button
            ...
            try {
                ...
                // Use JS executor for reliability (especially if the native click fails)
                ...
                // ⭐ NEW CRITICAL PAUSE: Give the UI a moment (1000ms) to begin the DOM removal process 
                // after the asynchronous server response. This addresses the TimeoutException.
                try { Thread.sleep(1000); } catch (InterruptedException e) { Thread.currentThread().interrupt(); }
        ...
        public void editChargeByName(String chargeName, ProjectDataModal.Charge updatedChargeData) {
            ...
            // ⭐ FIX: Locate the item's row based on its name, specifically targeting the LAST occurrence.
            // And now restricting the match to the first column (td[1]) only.
            String itemRowXPath = 
                "(//td[1][normalize-space(text())='%s']/ancestor::tr)[last()]";
            By itemRowLocator = By.xpath(String.format(itemRowXPath, chargeName));
            ...
            // Construct the locator for the action menu (three dots) - targeting the last row
            String actionButtonXPathTemplate = 
                itemRowXPath + "//td[last()]//button"; 
            ...
            try {
                // ⭐ NEW: Scroll the target row into the center of the viewport before interacting.
                System.out.println("ACTION: Scrolling to make the charge item visible for editing: " + chargeName);
                js.executeScript("arguments[0].scrollIntoView({block: 'center'});", itemRowElement);
